...
src/
  ...
  tools.rs            — all 28 tool implementations + definitions() + approval
  ...
  tui/                — ratatui TUI (declared as mod inside tui.rs)
    ...
    render.rs         — render_header/messages/input/status, model_pricing
...
| `app.view` | `ViewState` | messages, scroll, search, msg_line_offsets |
...
- **181 tests** — `cargo test` must stay green.
...
1. Bump `version` in both `Cargo.toml` and `package.json`
