...
use crate::{export_conversation, provider::{ApprovalPolicy, ChatMessage, PromptRequest}, unix_now};
    ...
    match text {
            ...
            match last {
                Some(text) => {
                    if super::render::write_to_clipboard(&text) {
                        app.view.messages.push(Msg::System("Last response copied to clipboard.".into()));
                    } else {
                        app.view.messages.push(Msg::Error("Could not write to clipboard (pbcopy/xclip/wl-copy not found).".into()));
                    }
                }
                None => app.view.messages.push(Msg::System("No assistant response to copy yet.".into())),
            }
            app.kbd.input.clear();
            app.kbd.input_cursor = 0;
            true
        ...
        "/undo" => {
            match app.conv.undo_stack.pop() {
                None => app.view.messages.push(Msg::System("Nothing to undo.".into())),
                Some((path, Some(old_content))) => {
                    match std::fs::write(&path, &old_content) {
                        Ok(()) => app.view.messages.push(Msg::System(format!("Restored {path}"))),
                        Err(e) => app.view.messages.push(Msg::Error(format!("Undo failed: {e}"))),
                    }
                }
                Some((path, None)) => {
                    // File was newly created — delete it
                    match std::fs::remove_file(&path) {
                        Ok(()) => app.view.messages.push(Msg::System(format!("Deleted {path} (undo create)"))),
                        Err(e) => app.view.messages.push(Msg::Error(format!("Undo failed: {e}"))),
                    }
                }
            }
        ...
        "/compact" => {
            ...
            if total_turns <= keep {
                ...
                let dropped_text: String = app.conv.history[..drop_msgs].iter()
                ...
                app.conv.history.insert(0, crate::provider::ChatMessage::user(format!(
                ...
                tokio::spawn(async move {
                    let (dummy_tx, _) = tokio::sync::mpsc::unbounded_channel::<crate::provider::StreamEvent>();
                    ...
                    if let Ok(result) = crate::provider::ask(
                        ...
                        if !result.text.is_empty() {
                            let _ = tx.send(TuiEvent::SystemMsg(
                                format!("Compact summary: {}", result.text.trim())
                            ));
                        }
                    }
                });
        ...
        s if s.starts_with("/export") => {
            ...
            match export_conversation(&path, &app.conv.history) {
                Ok(()) => app.view.messages.push(Msg::System(format!("Exported → {}", path.display()))),
        ...
        s if s.starts_with("/model") => {
            ...
            if arg.is_empty() {
                ...
                app.view.messages.push(Msg::System(format!("current model: {current}")));
            } else {
                app.model = arg.to_string();
                app.options.model = Some(arg.to_string());
                app.view.messages.push(Msg::System(format!("switched to model: {arg}")));
            }
            app.kbd.input.clear();
            app.kbd.input_cursor = 0;
            true
        ...
        s if s.starts_with("/provider") => {
            ...
            if arg.is_empty() {
                ...
                app.view.messages.push(Msg::System(format!("current provider: {current}")));
            } else {
                ...
                if app.config.providers.contains_key(arg) {
                    ...
                    app.view.messages.push(Msg::System(format!("switched to provider: {arg}")));
                } else {
                    app.view.messages.push(Msg::Error(format!("unknown provider '{arg}'")));
                }
            }
            app.kbd.input.clear();
            app.kbd.input_cursor = 0;
        ...
        s if s.starts_with("/add ") => {
            ...
            match fs::read_to_string(path) {
                Ok(content) => {
                    ...
                    let note = if truncated { " (truncated to 40k chars)" } else { "" };
                    let injected = format!(
                ...
                Err(e) => app.view.messages.push(Msg::Error(format!("Cannot read '{path_str}': {e}"))),
            }
            app.kbd.input.clear();
            app.kbd.input_cursor = 0;
            true
        ...
        "/diff" => {
            ...
            if diff.is_empty() {
                ...
                if staged.is_empty() {
                    app.view.messages.push(Msg::System("No changes (working tree clean).".into()));
                } else {
                    let capped: String = staged.chars().take(8_000).collect();
                    app.view.messages.push(Msg::System(format!("Staged diff:\n{capped}")));
                }
            } else {
                let capped: String = diff.chars().take(8_000).collect();
                app.view.messages.push(Msg::System(format!("git diff HEAD:\n{capped}")));
            }
            app.kbd.input.clear();
            app.kbd.input_cursor = 0;
            true
        ...
        s if s == "/commit" || s.starts_with("/commit ") => {
            ...
            if status.is_empty() {
                app.view.messages.push(Msg::System("Nothing to commit (working tree clean).".into()));
                app.kbd.input.clear();
                app.kbd.input_cursor = 0;
                return true;
            }

            let commit_msg = if s == "/commit" { None } else {
            ...
            if let Some(msg) = commit_msg {
                ...
                match Proc::new("git").args(["add", "-A"]).status() {
                    Ok(s) if s.success() => {}
                    _ => {
                        app.view.messages.push(Msg::Error("git add -A failed.".into()));
                        app.kbd.input.clear();
                        app.kbd.input_cursor = 0;
                        return true;
                    }
                ...
                match Proc::new("git").args(["commit", "-m", &msg]).output() {
                    Ok(out) if out.status.success() => {
                        ...
                        app.view.messages.push(Msg::System(format!("Committed: {msg}\n{}", stdout.trim())));
                    }
                    Ok(out) => {
                        let stderr = String::from_utf8_lossy(&out.stderr);
                        app.view.messages.push(Msg::Error(format!("git commit failed: {}", stderr.trim())));
                    }
                    Err(e) => app.view.messages.push(Msg::Error(format!("git commit error: {e}"))),
                }
            } else {
                ...
                tokio::spawn(async move {
                    ...
                    let request = PromptRequest {
                        ...
                        model: None, system: None, workspace_context: None,
                        history: vec![], images: vec![], mcp: None,
                    };
                    if let Ok(result) = crate::provider::ask(
                        &config, &provider_name, request,
                        ApprovalPolicy::Deny, &dummy_tx,
        ...
        s if s.starts_with("/memory ") => {
            ...
            match fs::OpenOptions::new().create(true).append(true).open(path) {
                Ok(mut f) => {
                    ...
                    app.view.messages.push(Msg::System(format!("Saved to .anveesa.md: {note}")));
                }
                Err(e) => app.view.messages.push(Msg::Error(format!("Cannot write .anveesa.md: {e}"))),
            }
            app.kbd.input.clear();
            app.kbd.input_cursor = 0;
            true
